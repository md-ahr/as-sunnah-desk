...
- [ ] Install runtime and dev dependencies ([02 § summary](./02-tech-stack-decisions.md#summary))
- [ ] `pnpm dlx shadcn@latest init --template next --base base`; confirm
      `components.json` has `"base": "base"`
- [ ] Install required shadcn primitives per
...
- [ ] Create the rule files above; extend `AGENTS.md` with a short **project** block
      (architecture one-liner, `pnpm verify`, pointer to `docs/`) — keep the Next.js
      block `next dev` generates
- [ ] Optional: `.cursor/skills/portal/SKILL.md` — indexes `docs/` by phase; do **not**
...
- [ ] `eslint.config.mjs` — extend `eslint-config-next/core-web-vitals` +
      `typescript-eslint` **strict-type-checked**
- [ ] `eslint-plugin-jsx-a11y` — explicit recommended rules
- [ ] `import/no-restricted-paths` — encode every row of the layer table from
...
- [ ] `eslint-config-prettier` — **last** in the config array
- [ ] `server-only` convention: every file under `src/server/` imports `'server-only'`;
      ESLint override in `src/app/**` and client files blocking `@/server/db` and
      `@/server/repositories`

### 0.4 · Prettier, EditorConfig, and VS Code

- [ ] `prettier.config.mjs` — `singleQuote`, `semi: false`, `trailingComma: 'all'`,
      `printWidth: 100`
- [ ] `prettier-plugin-tailwindcss`
- [ ] `.prettierignore` — `.next`, `drizzle/meta`, `pnpm-lock.yaml`,
      `src/components/ui/**` (optional)
- [ ] `.editorconfig` — `utf-8`, `lf`, `indent_size = 2`, `insert_final_newline = true`
- [ ] `.vscode/settings.json` — format on save, ESLint fix on save, workspace TS SDK
- [ ] `.vscode/extensions.json` — ESLint, Prettier, Tailwind CSS IntelliSense, Playwright,
      EditorConfig
...
- [ ] `.github/workflows/verify.yml` — runs `pnpm verify` on push/PR
      ([15 § CI workflow](./15-local-setup.md#ci-workflow))
- [ ] `pnpm verify` = `typecheck` → `lint` → `knip` → `test:run` → `build` →
...
- [ ] `"paths": { "@/*": ["./src/*"] }`, Next plugin, `"moduleResolution": "bundler"`
- [ ] `tsconfig.test.json` — Vitest globals, `src/test/**`
- [ ] `typecheck` script: `next typegen && tsc --noEmit`
- [ ] `@typescript-eslint/explicit-module-boundary-types` on `src/server/**` and
      `src/features/**/actions/**` only

### 0.7 · Dead code and dependency hygiene (Knip)

- [ ] `knip` + `knip.json` — Next.js plugin, explicit entry points for scripts and configs
- [ ] `ignore`: `src/components/ui/**`, `.next/**`, `drizzle/meta/**`
- [ ] Include `pnpm knip` in `pnpm verify` after `lint`, before `test:run`

### 0.8 · Repository hygiene

- [ ] `.nvmrc` or `"engines": { "node": ">=22" }`
- [ ] `.npmrc`: `strict-peer-dependencies=true`
- [ ] `.env.example` per [15 § environment variables](./15-local-setup.md#environment-variables)
- [ ] `db:*` scripts per [15 § database scripts](./15-local-setup.md#database-scripts)
- [ ] `README.md` stub: prerequisites, `pnpm install`, `lefthook install`, `db:migrate`,
      `db:seed`, test credentials, `pnpm verify`

### Done when

- [ ] `pnpm verify` passes on an empty application
- [ ] A layer-boundary import **fails ESLint** with a clear message
- [ ] Format-on-save and Lefthook pre-commit / pre-push work
- [ ] Cursor rules are active; agent can link to `docs/` by phase
